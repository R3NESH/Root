...
severity: high
---
# The client-side fallback claimed rules it never enforced

> [!success] Fixed 2026-08-31 — the labelling half
> `solveClientSide()` now returns `status: OFFLINE_ESTIMATE` (exported as
> `OFFLINE_ESTIMATE_STATUS`, which the ribbon keys off), `rules_applied: []`,
> `rooms_reachable: 1` — the honest count for an empty door graph — and `rules_relaxed: true`.
...
> a **timeout** as well as on INFEASIBLE, so a loaded machine can drop the zone rules server-side too.
> Measured 10/10 on a ten-room mix at a 50 ms interactive budget. The response now carries
> `meta.rules_relaxed` and the same badge warns on it.
...
- returns `status: "Solved (Optimal)"`;
- returns four `rules_applied` entries, all invented, none of which any line in the function
  enforces. The real solver emits a rule's text only after `add_quadrant_constraint()` has
...
rules-as-constraints is the decision that a violating plan is *rejected*, not merely worse.
A plan that quietly violates a posted rule while displaying "Solved (Optimal)" is not a degraded
...
1. Have `solveClientSide()` return `status: "OFFLINE_ESTIMATE"` and
   `rules_applied: []`, and surface that in the header the way `staleBackend`
   already is.
...
Related: [[duplicated-geometry]] · rules-as-constraints · [[output-schema]] ·
[[project-status]]

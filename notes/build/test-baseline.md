...
| 2026-08-30 | **43** | 0 | none | after fixing a NE quadrant rule, small custom room ladder bounds, and entrance room door; `pytest -q`, 21.4s wall |
| 2026-08-31 | **50** | 0 | none | after the audit fixes: room semantics across the API boundary, and `rules_relaxed`; `pytest -q`, 25.6s wall |
| 2026-09-03 | **93** | 0 | none | after walls as objects, quantities BOQ, NBC 2016 room sizes, compact footprint objective, programs registry, prompt-to-plan, and real Kandi plot validation; `pytest` |

**Delta: +43 tests, 0 failures.** Composition as of 2026-09-03: `test_api.py` **14** ·
`test_kandi_plot.py` **3** · `test_programs.py` **23** · `test_prompt_to_plan.py` **3** ·
`test_realism.py` **15** · `test_solver.py` **6** · `test_stability.py` **4** · `test_zoning.py` **13** ·
`test_walls.py` **12**.

The forty-three added on 2026-09-03 encompass:
- `test_kandi_plot.py` **+3** — integration tests on the actual 30×40 North-facing plot in Kandi, Telangana under TG-bPASS setback rules (5 ft front, 3 ft sides/rear). Verifies standard 2BHK with a store and two baths (100% reachability, BIM walls and BOQ takeoff), tight compact footprint, and proves 3BHK feasibility under NBC 2016 sizing.
...
- `test_programs.py` **+23** — multi-program zoning registry (residence vs café flow topology, hub connectivity, forbidden adjacencies).
...
- `test_zoning.py` **+5** — a solve that drops its zone rules must say so (`rules_relaxed`), and the
...
> 25 green tests do not notice that Zone rules and connectivity are switched off on every solve
> after the first — see the drag regression. `test_zoning.py` never passes
> `prev`; `test_stability.py:80` passes `prev` with zone rules on but asserts only status and time.
> **Two invariants were missing.** Both were added on 2026-08-25 and both now pass:
> `test_rules_still_hold_when_prev_is_supplied` and

...
master bedroom, store and stair off the dining space — with a fan-out cap of five. Connectivity
...
- **Zone rules and the tree contradicted each other.** The fan-out cap pushed a ruled room onto
  the kitchen — north-east and south-east, opposite corners. On the real buildable envelope of a
  30×40 plot this took zone rules from three applied to **zero**, because the model became
  infeasible and the ladder walked down to a rung with zone rules off. On the plot size this product
...
8 rooms reachable**. Relaxation now goes: custom sizes → daylight → zone rules → the area
...
| Cold solve, real 30×40 2BHK | — | **OPTIMAL in ~1.0 s**, all 3 zone rules |
...
- the direction-rules step's entrance gap is closed: the front door exists, prefers N→E, and now
...
- **Four room kinds were removed again the same day** — parking porch, sit-out, staircase and
  utility — leaving hall, dining, kitchen, bedroom, bathroom and store. The `open_sided`
...
**Links.** [[rooms-do-not-form-a-house]] · the drag regression ·
